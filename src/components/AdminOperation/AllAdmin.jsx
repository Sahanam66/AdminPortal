...
const AllAdmin = () => {
 ...
 axios.get('  http://localhost:4001/Admin')
    .then((resp)=>  {
  ...
  let deletedata=(id)=>{
    ...
    axios.delete(` http://localhost:4001/Admin/${id}`)
    .then((resp)=>{
        ...
        <TableBody>
          {rows.map((row) => (
            <StyledTableRow key={row.id}>
              ...
              <StyledTableCell align="center" onClick={()=>{navigate(`/editAdmin/${row.id}`)}} style={{cursor:'pointer'}}><EditIcon/></StyledTableCell>
